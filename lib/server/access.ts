...
export async function requirePartner(){const auth=await requireUser();const {data,error}=await auth.supabase.from("partner_memberships").select("id,organization_id,role,status,partner_organizations(id,name,type,status,trusted_generation)").eq("user_id",auth.user.id).eq("status","ACTIVE").limit(1).maybeSingle();if(error||!data)throw new ForbiddenError();return {...auth,membership:data};}
