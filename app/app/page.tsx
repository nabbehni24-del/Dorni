...
export default function OwnerApp(){
  ...
  async function createTicket(){if(await post("/api/support",{category:"GENERAL",...ticket})){setTicket({subject:"",description:""});setNotice("تم حفظ تذكرة الدعم وإرسالها لفريق دورني");}}
  ...
  {panel?<section className="work-card settings-panel"><Button variant="ghost" onClick={()=>setPanel(null)}>رجوع للحساب</Button>{panel==="support"&&<><h2>افتح تذكرة دعم</h2><p>صف المشكلة باش يقدر فريق الدعم يتابعها.</p><Input placeholder="عنوان المشكلة" value={ticket.subject} onChange={e=>setTicket({...ticket,subject:e.target.value})}/><Textarea placeholder="اشرح المشكلة" value={ticket.description} onChange={e=>setTicket({...ticket,description:e.target.value})}/><Button onClick={createTicket} disabled={busy||!ticket.subject||!ticket.description}>إرسال للدعم</Button></>}{panel==="guide"&&<div className="guide-steps"><h2>من البطاقة إلى التنبيه</h2><ol><li>أضف سيارتك.</li><li>اكشط منطقة رمز المطالبة في بطاقة دورني.</li><li>أدخل الرقم التسلسلي ورمز المطالبة لتفعيل البطاقة.</li><li>ثبّت البطاقة في مكان واضح.</li><li>أي شخص يمسح QR يختار سبباً جاهزاً، من غير ما يشوف رقمك.</li></ol></div>}{panel==="privacy"&&<><h2>خصوصيتك هي الأساس</h2><p>الماسح يرى نوع السيارة ولونها فقط. لا نعرض اسمك أو رقم هاتفك أو واتساب أو أي وسيلة تواصل مباشرة. روابط متابعة البلاغ مؤقتة وغير قابلة للتخمين.</p></>}{panel==="settings"&&<><h2>التطبيق والتنبيهات</h2><p>التنبيهات تتحدّث تلقائياً كل 5 ثوانٍ ودورني مفتوح ومتصل بالإنترنت.</p><p>إشعارات شاشة القفل لم تُفعّل بعد. سيظهر زر تفعيلها عند إطلاق الخدمة.</p><Link href="/install">تثبيت دورني على التلفون</Link></>}</section>:<>
